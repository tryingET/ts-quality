...
function renderEvidenceClosurePromptMarkdown(action) {
    const lines = [
        '---',
...
function renderEvidenceClosureTaskManifest(action) {
    return `${(0, index_1.stableStringify)(action.primaryAction.taskManifest)}\n`;
}
...
function isCombinedConditionMutation(survivor) {
...
function assertionStrategyForMutation(survivor) {
    if (isBoundaryMutation(survivor)) {
...
function maskingRiskForMutation(survivor) {
...
function enclosingSymbolForMutation(symbols, survivor) {
    if (!survivor.span) {
        return undefined;
    }
    const filePath = (0, index_1.normalizePath)(survivor.filePath);
    const line = survivor.span.startLine;
    const candidates = symbols
        .filter((symbol) => (0, index_1.normalizePath)(symbol.filePath) === filePath && symbol.span.startLine <= line && symbol.span.endLine >= line)
        .sort((left, right) => (left.span.endLine - left.span.startLine) - (right.span.endLine - right.span.startLine));
    return candidates[0]?.symbol;
}
function focusedTestsForFile(run, filePath) {
    const normalized = (0, index_1.normalizePath)(filePath);
    return uniqueStrings(run.behaviorClaims
        .filter((claim) => (claim.evidenceSummary?.impactedFiles ?? []).map((item) => (0, index_1.normalizePath)(item)).includes(normalized))
        .flatMap((claim) => claim.evidenceSummary?.focusedTests ?? []));
}
function survivorGroupKey(survivor) {
    return [
        (0, index_1.normalizePath)(survivor.filePath),
        survivor.span?.startLine ?? 'unknown',
        survivor.original ?? survivor.operator ?? 'unknown',
        survivor.replacement ?? survivor.mutated ?? 'unknown'
    ].join(':');
}
function expectedConfidenceLiftFor(run, actionKind) {
...
function buildPrimaryEvidenceClosureAction(run, remainingBlocker, artifactPaths) {
    ...
    if (surviving.length > 0) {
        const targetFiles = uniqueStrings(surviving.map((item) => (0, index_1.normalizePath)(item.filePath)));
        ...
        const steps = groupEntries.map(([, group], index) => {
            ...
            const survivorTargetFiles = uniqueStrings(group.map((item) => (0, index_1.normalizePath)(item.filePath)));
...
function runCheck(rootDir, options) {
    ...
    if (run.nextEvidenceAction) {
        ...
        fs_1.default.writeFileSync(path_1.default.join(artifactDir, 'next-evidence-action.prompt.md'), renderEvidenceClosurePromptMarkdown(run.nextEvidenceAction), 'utf8');
        fs_1.default.writeFileSync(path_1.default.join(artifactDir, 'next-evidence-action.ak-task.json'), renderEvidenceClosureTaskManifest(run.nextEvidenceAction), 'utf8');
    }
