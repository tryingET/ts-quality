...
function renderEvidenceClosurePromptMarkdown(action: NextEvidenceAction): string {
  const lines = [
    '---',
...
function renderEvidenceClosureTaskManifest(action: NextEvidenceAction): string {
...
function survivorGroupKey(survivor: RunArtifact['mutations'][number]): string {
  return [
...
function expectedConfidenceLiftFor(run: Pick<RunArtifact, 'verdict'>, actionKind: NextEvidenceAction['primaryAction']['kind']): number | undefined {
...
function buildNextEvidenceAction(run: Pick<RunArtifact, 'runId' | 'coverage' | 'coverageGeneration' | 'executionWitnesses' | 'files' | 'mutations' | 'governance' | 'verdict' | 'behaviorClaims' | 'symbols'>): NextEvidenceAction {
...
function readPackageManager(rootDir: string): PackageManagerName {
  const packagePath = path.join(rootDir, 'package.json');
  if (fs.existsSync(packagePath)) {
    try {
...
function coverageScriptNames(scripts: Record<string, string>): string[] {
...
function buildDoctorDiagnostic(rootDir: string, options?: { changedFiles?: string[]; configPath?: string }): DoctorDiagnostic {
  ...
  const changedOutsideSources = changed.filter((filePath) => !sourcePatterns.some((pattern) => matchPattern(pattern, filePath)));
