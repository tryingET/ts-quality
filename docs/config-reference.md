...
`coverage.lcovPath` defaults to `coverage/lcov.info`. If that file is missing during `check` and `coverage.generateCommand` is configured, `check` first creates the parent directory for `coverage.lcovPath`, then runs the command from the repo root, requires it to pass, requires `coverage.lcovPath` to exist afterward, and parses the generated LCOV before CRAP, mutation, invariant, and governance evaluation. The run artifact records an additive `coverageGeneration` receipt, stdout plus `check-summary.txt` show the generation status, and the run directory also receives `coverage-generation.json` / `coverage-generation.txt` sidecars.
...
If `generateCommand` is configured and fails, or if it passes without creating the expected LCOV file, `check` fails closed instead of silently continuing with empty coverage evidence. Choose the command the same way you choose witness proof commands: prefer a target-repo script that runs the relevant tests and writes LCOV, and put any required build step before the command or inside the script when tests execute built output. For TypeScript repos whose LCOV reports built files such as `dist/**/*.js` while changed files are under `src/**/*.ts`, enable source-map coverage mapping (for example `NODE_OPTIONS=--enable-source-maps`) or otherwise configure coverage to map back to `src/**`; `check` now emits an analysis warning when it detects built-output coverage without changed-source coverage.
...
`check` runs that command for impacted scenarios, writes the witness under the configured repo-local output path, writes a sibling `.receipt.json` execution receipt sidecar, then lets invariant evaluation consume it as execution-backed support. Manual witnesses written under `.ts-quality/witnesses/**/*.json` are also consumed by `check` without scenario config duplication when they match invariant id, scenario id, pass status, and impacted source scope; use explicit `executionWitnessPatterns` only for non-default witness locations or narrower matching. Choose `executionWitnessCommand` the same way you choose the command after manual `witness test --`: prefer a module-level target-repo proof command tied to `executionWitnessTestFiles`, use repo-global `npm test` only as baseline evidence when it cannot focus the slice, and wrap long TypeScript/source-mode loader commands in a repo-local npm script. If the command imports built output, put the target repo build in the CI stage before witness refresh/check. The repo-native pre-refresh surface is `npx ts-quality witness refresh` (or `npm run witness:refresh --silent` from this repo), which runs the same configured impacted witness commands before `check` when you want that stage to stay explicit in CI. Keep first-invariant authoring guidance in `docs/adoption/first-invariant-witness-authoring.md`; keep the witness artifact/receipt semantics in `docs/invariant-dsl.md`; keep CI staging guidance in `docs/ci-integration.md`.
...
- `mutations.runtimeMirrorRoots` tells mutant runs which built-runtime roots should mirror mutated sources into executable runtime trees (default: `['dist']`). JS sources are copied directly; TS/TSX sources are transpiled before being written into matching runtime mirror files. Use this when tests execute built output from `dist/`, `lib/`, `build/`, or another runtime tree. Root-level source files are also mirrored into matching built roots such as `dist/index.js`. The same roots inform the built-output LCOV warning when changed source files under `src/**` are not covered directly. In workspaces, name package-local runtime roots such as `packages/api/dist` or `packages/api/lib` when tests import package-local built output; pair this with source-map LCOV that reports authored package source and a hunk-scoped `changeSet.diffFile` when validating complex monorepo slices. See `docs/adoption/2026-05-06-complex-typescript-monorepo-source-map-validation-pilot.md` for a practical pilot checklist.
