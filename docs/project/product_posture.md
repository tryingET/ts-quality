...
`ts-quality` already has the core deterministic trust layers in place — scoped evidence, generated/consumed coverage evidence, mutation pressure, invariant reasoning, governance, legitimacy, run-bound artifacts, public npm release, Trusted Publishing provenance, representative installed-package proof, production-clean npm README presentation, first-user CLI help, and two temp-copy outside-repo first-witness pilots — but its main maturity gap is turning those layers into repeated outside-repo adoption where fresh operators can keep witness-backed reviews habitual across real repos without repo-internal scaffolding.
...
- The package path is no longer just conceptual; staged packaging, public npm publication, Trusted Publishing provenance, production-clean npm README presentation, first-user CLI help including the first focused witness path, configured LCOV generation, representative installed-package review/governance/legitimacy smoke proof, and two temp-copy outside-repo first-witness pilots exist.

## Current gaps

- The outside-repo operator story is not yet as mature as the repo-internal architecture.
- Outside-repo adoption has two temp-copy target-repo pilots, plus a pi-server rerun proving configured LCOV generation can clear missing coverage and CRAP pressure, but still needs proof beyond maintainer-guided setup and better target-repo proof-command / mutation-remediation selection.
...
A release operator can now repeat representative review/governance/legitimacy flows from an installed package in fixture-backed smoke proof, npm now presents the production-clean README, the CLI/README teach the first bounded-review plus first-witness contract directly, and two temp-copy outside-repo pilots proved the witness upgrade path against `designmd-foundry` and `pi-server`. The next product-readiness step is reducing setup/witness friction enough that the habit works in real outside repos without maintainer narration.
