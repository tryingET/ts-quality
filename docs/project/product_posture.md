...
- Manual `witness test` artifacts are visible in the first-user CLI/README path, and public `0.2.2` makes matching `.ts-quality/witnesses/**/*.json` pass witnesses first-class `check` evidence; the remaining proof gap is broader target-shape validation, not core witness consumption.
...
1. Add schema-version compatibility fixtures for older `0.1.0` run artifacts and current `0.2.x` artifacts so downstream agents and parsers can survive additive artifact growth.
