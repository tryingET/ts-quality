...
- The package path is no longer just conceptual; staged packaging, public npm publication through `0.2.0`, Trusted Publishing provenance, production-clean npm README presentation, first-user CLI help including the first focused witness path, configured LCOV generation, compact `doctor --machine` diagnostics on `main`, representative installed-package review/governance/legitimacy smoke proof, and two temp-copy outside-repo first-witness pilots exist.

## Current gaps

- The outside-repo operator story is not yet as mature as the repo-internal architecture.
- Outside-repo adoption has two temp-copy target-repo pilots, plus a pi-server rerun proving configured LCOV generation can clear missing coverage and CRAP pressure; the attempted public `0.2.0` pilot found that `doctor --machine` was not actually in the published tarball, so the next public pilot must use the next release that contains compact diagnostics and release-verifier drift checks.
...
A release operator can now repeat representative review/governance/legitimacy flows from an installed package in fixture-backed smoke proof, npm presents the production-clean README, the CLI/README teach the first bounded-review plus first-witness contract directly, `doctor --machine` gives harnessed LLMs a compact setup diagnostic on `main`, and two temp-copy outside-repo pilots proved the witness upgrade path against `designmd-foundry` and `pi-server`. The next product-readiness step is publishing and proving that compact-diagnostics loop in real outside repos without maintainer narration.

## Near-term convergence path

1. Keep release verification fail-closed on critical public CLI contracts, including compact `doctor --machine`, so docs cannot claim an adoption path before the tarball exposes it.
2. Run the next public outside-repo pilot from the release that contains `doctor --machine`, uses an init preset where appropriate, and records whether the agent/operator can reach `check`, `explain`, and one focused witness without hidden repo memory.
3. Validate that the new mutation remediation, next-evidence-action, built-output LCOV warning, and coverage-generation sidecars reduce the pi-server-style adoption friction.
4. Add schema-version compatibility fixtures for older `0.1.0` run artifacts and current `0.2.0` artifacts so downstream agents and parsers can survive additive artifact growth.
