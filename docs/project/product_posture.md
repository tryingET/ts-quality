...
`ts-quality` already has the core deterministic trust layers in place — scoped evidence, generated/consumed coverage evidence, mutation pressure, invariant reasoning, governance, legitimacy, run-bound artifacts, public npm release through `0.3.1`, Trusted Publishing provenance, representative installed-package proof, production-clean npm README presentation, first-user CLI help, compact public `doctor --machine` diagnostics, four outside-repo adoption pilots, public-package proof that manual focused witnesses become `check` evidence, public release proof for the 0.3 evidence contract baseline, and a green patch release for the public verifier parser — but its main maturity gap is broader compatibility coverage across real target repo shapes as the public evidence surface grows.
...
- The package path is no longer just conceptual; staged packaging, public npm publication through `0.3.1`, Trusted Publishing provenance, production-clean npm README presentation, first-user CLI help including the first focused witness path, configured LCOV generation, compact public `doctor --machine` diagnostics, representative installed-package review/governance/legitimacy smoke proof, public release verification for manual witness evidence and the 0.3 contract baseline, and four outside-repo adoption pilots exist.
...
A release operator can now repeat representative review/governance/legitimacy flows from an installed package in fixture-backed smoke proof, npm presents the production-clean README, the CLI/README teach the first bounded-review plus first-witness contract directly, public `doctor --machine` gives harnessed LLMs a compact setup diagnostic, the public `0.2.1` pi-server pilot proved the compact-diagnostics loop through generated coverage, mutation remediation, and scenario-configured execution-backed witness evidence, public `0.2.2` proved the manual `witness test` artifact path is first-class evidence for `check` without duplicated scenario config, public `0.2.3` protected that path plus legacy artifact projection compatibility in release verification, and public `0.3.1` now ships the 0.3 evidence contract baseline with a green public verifier. The `0.3.x` line should now broaden compatibility/adoption proof rather than adding feature breadth.
...
2. Continue outside-repo pilots across additional target shapes, now focusing on whether the public `doctor --machine` -> manual `witness test` -> `check` -> `report`/`explain` story remains understandable without maintainer narration.
