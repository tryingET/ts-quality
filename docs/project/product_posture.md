...
`ts-quality` already has the core deterministic trust layers in place — scoped evidence, generated/consumed coverage evidence, mutation pressure, invariant reasoning, governance, legitimacy, run-bound artifacts, public npm release through `0.2.3`, Trusted Publishing provenance, representative installed-package proof, production-clean npm README presentation, first-user CLI help, compact public `doctor --machine` diagnostics, four outside-repo adoption pilots, public-package proof that manual focused witnesses become `check` evidence, main-branch release-gate proof for manual witnesses plus legacy artifact projections, and a documented 0.3 public contract baseline — but its main maturity gap is broader compatibility coverage across real target repo shapes as the public evidence surface grows.
...
| Execution witnesses | Witness commands, witness artifacts, receipt sidecars, scenario-configured auto-run witnesses, and public `0.2.2` default discovery for manual `.ts-quality/witnesses/**/*.json` pass witnesses exist; the public `0.2.1` pi-server pilot proved auto-run witnesses are recognized by `check`, and the public `0.2.2` proof verified manual witnesses are consumed without duplicated scenario config. | Important invariant scenarios can graduate from deterministic lexical evidence to explicit proof artifacts. | The witness path now needs continued adoption validation in more target shapes, not another core semantic correction. | A fresh repo can create one manual focused witness from the public package, rerun `check`, and see execution-backed support without duplicating the witness path/command in invariant config. |
...
| Artifact contract | `run.json`, verdict/report artifacts, decision context, authorization outputs, amendment JSON/text, PR summaries, trend output, sample artifacts, installed-package compatibility proof for legacy `0.1.0` run projections, and `docs/public-contract.md` exist. | Artifacts behave like an audit packet: machine-stable, human-inspectable, and projection-aware. | Compatibility proof now needs breadth across more historical/current artifact shapes, not just one legacy projection fixture. | Concise Markdown and JSON artifacts tell the same story, with no competing authority, across legacy and current run packets. |
...
| Packaged outside-repo use | Public npm publishing works through GitHub Release + Trusted Publishing/OIDC through `0.2.3`; `smoke:packaging` proves installed CLI/API/types plus representative review, governance, legitimacy, materialized-config, drift, monorepo fixture flows, manual witness consumption, legacy artifact projection, and the shared public CLI/evidence contract; npm shows the production-clean README; `docs/adoption/fourth-witness-pilot-0.2.1.md` records the public compact-diagnostics pilot plus the follow-up public manual-witness proof. | Installed package use should feel production-clean from the first public page through first bounded review. | The compact doctor/release-contract gap and manual witness-consumption gap are closed; the next adoption gap is proving the same flow across additional target repo shapes and expanding schema compatibility fixtures. | A fresh target repo follows a minimal bounded review plus one manual focused witness from the public package, and the resulting artifacts are understandable without maintainer narration. |
...
- The package path is no longer just conceptual; staged packaging, public npm publication through `0.2.3`, Trusted Publishing provenance, production-clean npm README presentation, first-user CLI help including the first focused witness path, configured LCOV generation, compact public `doctor --machine` diagnostics, representative installed-package review/governance/legitimacy smoke proof, public release verification for manual witness evidence, and four outside-repo adoption pilots exist.
...
- Manual `witness test` artifacts are visible in the first-user CLI/README path, public `0.2.2` makes matching `.ts-quality/witnesses/**/*.json` pass witnesses first-class `check` evidence, and main now protects that path in staged/public release verification; the remaining proof gap is broader target-shape validation, not core witness consumption.
- Concise summaries must keep improving without becoming a second authority over `run.json`.
- Adoption docs, sample artifacts, release-note breaking-change / agent-migration guidance, and compatibility fixtures must stay synchronized with shipped artifact schema changes.
...
A release operator can now repeat representative review/governance/legitimacy flows from an installed package in fixture-backed smoke proof, npm presents the production-clean README, the CLI/README teach the first bounded-review plus first-witness contract directly, public `doctor --machine` gives harnessed LLMs a compact setup diagnostic, the public `0.2.1` pi-server pilot proved the compact-diagnostics loop through generated coverage, mutation remediation, and scenario-configured execution-backed witness evidence, public `0.2.2` proved the manual `witness test` artifact path is first-class evidence for `check` without duplicated scenario config, and public `0.2.3` protects that path plus legacy artifact projection compatibility in release verification. The `0.3.x` line should now make those contracts explicit and broaden compatibility/adoption proof rather than adding feature breadth.

## Near-term convergence path

1. Expand schema-version compatibility fixtures beyond the first legacy `0.1.0` projection proof, especially current `0.2.x` artifacts with additive fields and future candidate fields.
2. Continue outside-repo pilots across additional target shapes, now focusing on whether the public manual `witness test` -> `check` path remains understandable without maintainer narration.
3. Keep witness discovery narrow: invariant id, scenario id, pass status, and impacted source scope must all match before manual artifacts count as execution-backed support.
