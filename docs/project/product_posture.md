...
`ts-quality` already has the core deterministic trust layers in place — scoped evidence, generated/consumed coverage evidence, mutation pressure, invariant reasoning, governance, legitimacy, run-bound artifacts, public npm release through `0.2.2`, Trusted Publishing provenance, representative installed-package proof, production-clean npm README presentation, first-user CLI help, compact public `doctor --machine` diagnostics, four outside-repo adoption pilots, public-package proof that manual focused witnesses become `check` evidence, and main-branch release-gate proof for manual witnesses plus legacy artifact projections — but its main maturity gap is broader compatibility coverage across real target repo shapes as the public evidence surface grows.
