...
- The package path is no longer just conceptual; staged packaging, public npm publication, Trusted Publishing provenance, production-clean npm README presentation, first-user CLI help including the first focused witness path, representative installed-package review/governance/legitimacy smoke proof, and two temp-copy outside-repo first-witness pilots exist.

## Current gaps

- The outside-repo operator story is not yet as mature as the repo-internal architecture.
- Outside-repo adoption has two temp-copy target-repo pilots, but still needs proof beyond maintainer-guided setup and better target-repo proof-command selection.
...
2. Validate the improved post-witness residual-pressure wording from `0b4e59f` in the next public release/pilot cycle.
