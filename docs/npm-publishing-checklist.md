...
Local Pi/orchestrator sessions prepare proof and release notes; npm publication happens automatically in `.github/workflows/publish.yml` through npm Trusted Publishing/OIDC after a GitHub Release is published. Prerelease GitHub Releases publish with npm dist-tag `next`; other releases publish with `latest`.
...
- **npm Trusted Publishing/OIDC is publication authority** — maintainers must configure the npm package trusted publisher for GitHub owner `tryingET`, repository `ts-quality`, workflow filename `publish.yml`, and environment name `npm-publish`; no local npm token or local `npm publish` is part of the operator path
...
6. let `.github/workflows/publish.yml` run in the `npm-publish` environment, install Node `24` and npm `>=11.5.1`, verify GitHub OIDC runtime prerequisites, validate the tag/version contract, re-run `npm run verify:ci`, upload the proven tarball artifact, publish to npm with `npm publish --provenance --access public` through Trusted Publishing/OIDC, start the public-propagation timer only after publish succeeds, retry public `npm view ts-quality@<next-version> version` exact-version visibility separately from public `npx -p ts-quality@<next-version>` checks for help, compact doctor diagnostics, and the minimal manual `witness test` -> `check` evidence path, and attach the proven tarball to the GitHub Release after publish succeeds
7. run `npm run release:verify-public -- --version <next-version>` after the workflow succeeds; this repo-local verifier bypasses npm `min-release-age`, retries its own exact-version registry lookup and public-install checks, and reports whether npm propagation, CLI installability, or the manual-witness evidence contract exhausted
