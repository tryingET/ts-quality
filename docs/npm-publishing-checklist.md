...
6. let `.github/workflows/publish.yml` run in the `npm-publish` environment, install Node `24` and npm `>=11.5.1`, verify GitHub OIDC runtime prerequisites, validate the tag/version contract, re-run `npm run verify:ci`, upload the proven tarball artifact, publish to npm with `npm publish --provenance --access public` through Trusted Publishing/OIDC, start the public-propagation timer only after publish succeeds, retry public `npm view ts-quality@<next-version> version` exact-version visibility separately from `npx -p ts-quality@<next-version> ts-quality --help`, and attach the proven tarball to the GitHub Release after publish succeeds
7. run `npm run release:verify-public -- --version <next-version>` after the workflow succeeds; this repo-local verifier bypasses npm `min-release-age`, retries its own exact-version registry lookup and public-install checks, and reports whether npm propagation or CLI installability exhausted
