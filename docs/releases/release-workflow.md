...
8. after `npm publish` succeeds, starts a bounded public-propagation timer by retrying `npm view ts-quality@<version> version` until the exact version is visible, then runs the shared public CLI contract verifier (`scripts/public-cli-contract.mjs`) for public `npx` checks including `ts-quality --help` and `ts-quality doctor --machine --changed src/index.ts`, because npm packument visibility and `npx` install resolution can converge at different times immediately after publish and public CLI subcommand drift must fail closed
