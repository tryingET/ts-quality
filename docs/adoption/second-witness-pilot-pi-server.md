...
Follow-up slice `#1908` closed the remaining wording issue: high-level verdict reasons for execution-backed invariants now name residual evidence pressure directly, such as mutation pressure, instead of saying stronger test evidence or failure-path coverage is needed after the witness and coverage signals are already clear.
