...
| D4 | The coverage-generation failure message is escaped twice (`\u000a` and `\\u000a`) because the message is escaped when built and again by the CLI error renderer. | Fixed in #5899: the message is raised raw and escaped once at the CLI boundary, like other errors; that boundary is unchanged. |
