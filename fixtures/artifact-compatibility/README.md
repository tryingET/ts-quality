...
- `real-bun-esm/` — real large ESM service tested with the Bun runner, from `semantic-code-intelligence` (narrowed capture of the scale pilot; fail with genuine survivors and governance-blocked authorization); see `docs/adoption/2026-09-26-semantic-code-intelligence-bun-scale-adoption.md`. `src/core/runtime-config.ts` is Apache-2.0 code from that repository; its license is in `real-bun-esm/LICENSE`.
