...
7. `policy-engine` emits an explainable merge-confidence verdict with an additive arithmetic breakdown and explicitly blocks on invalid mutation baselines, mutation execution errors, and actionable surviving mutants.
...
11. Artifacts are persisted to `.ts-quality/runs/<run-id>/`, including optional execution-witness run summaries when configured witness plans were considered, coverage-generation sidecars when LCOV generation was attempted, mutation remediation payloads when survivors remain, and a next-evidence-action summary for the operator's immediate follow-up.
